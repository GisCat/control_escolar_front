...
  public userRole: string = '';
  public materiasDropdownOpen: boolean = false; 
...
    }
  };
// Método para saber si se dio clic 
  togglePalette() {
...
  constructor(private router: Router, private facadeService: FacadeService) { //inyectando el router y el servicio facade
...
  }
 O
  ...
  handleClickOutside(event: Event) {
      ...
      // Cerrar el menú del avatar/usuario si está abierto y el clic fue fuera del contenedor
      if (this.showUserMenu && !target.closest('.scoop-navbar-avatar')) {
          this.showUserMenu = false;
      }

      // Cerrar el dropdown de Materias si está abierto y el clic fue fuera del contenedor principal
      if (this.materiasDropdownOpen && !target.closest('.dropdown-parent')) {
          this.materiasDropdownOpen = false;
      }
  }
...
  }
  ...
  toggleMateriasDropdown(): void {
    this.materiasDropdownOpen = !this.materiasDropdownOpen;
  }
  
  closeMateriasDropdown(): void {
    this.materiasDropdownOpen = false;
  }
...
  }
  canSeeAdminItems(): boolean { // Solo el admin puede ver estos ítems
    return this.isAdmin();
  }
  canSeeTeacherItems(): boolean { // El admin y el maestro pueden ver estos ítems
    return this.isAdmin() || this.isTeacher();
  }
  canSeeStudentItems(): boolean {
    return this.isAdmin() || this.isTeacher() || this.isStudent();
  }
  canSeeHomeItem(): boolean { // El admin y el maestro pueden ver estos ítems
    return this.isAdmin() || this.isTeacher();
  }
  canSeeRegisterItem(): boolean { // El admin y el maestro pueden ver estos ítems
    return this.isAdmin() || this.isTeacher();
  }
